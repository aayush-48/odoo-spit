...
const ReceiptDetails = ({ receipt, open, onClose }: ReceiptDetailsProps) => {
  const { suppliers, warehouses, products } = useInventory();
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="font-mono text-sm font-semibold mt-1">{receipt.id}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="font-semibold mt-1">{getSupplierName(receipt.supplierId)}</p>
            </div>
            <div>
              <label className="text-sm font-medium text-muted-foreground">Warehouse</label>
              <p className="font-semibold mt-1">{getWarehouseName(receipt.warehouseId)}</p>
            </div>
            <div>
              <label className="text-sm font-medium text-muted-foreground">Created Date</label>
              <p className="text-sm mt-1">{format(new Date(receipt.createdAt), 'MMM dd, yyyy HH:mm')}</p>
            </div>
            <div>
              <label className="text-sm font-medium text-muted-foreground">Last Updated</label>
              <p className="text-sm mt-1">{format(new Date(receipt.updatedAt), 'MMM dd, yyyy HH:mm')}</p>
            </div>
            {receipt.notes && (
              <div className="col-span-2">
                <label className="text-sm font-medium text-muted-foreground">Notes</label>
                <p className="text-sm mt-1 bg-muted/50 p-3 rounded-lg">{receipt.notes}</p>
              </div>
            )}
          </div>
          ...
          <div className="border-t border-border pt-4">
            <h4 className="font-semibold text-foreground mb-4">
              Products ({receipt.lines.length} item{receipt.lines.length !== 1 ? 's' : ''})
            </h4>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-muted/50">
                    ...
                    {receipt.lines.some(l => l.unitPrice) && (
                ...
                <TableBody>
                  {receipt.lines.map((line) => (
                    <TableRow key={line.productId}>
                      ...
                      {receipt.lines.some(l => l.unitPrice) && (
                        <TableCell className="text-right font-semibold">
                          {line.unitPrice ? `$${(line.quantity * line.unitPrice).toFixed(2)}` : '-'}
                        </TableCell>
                      )}
                    </TableRow>
                  ))}
